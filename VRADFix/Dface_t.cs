...
namespace VRADFix
{
        ...
        
    {
        ...
        */ 
        ...
        Bsp bsp;
        public readonly int classLength = 56; // 0x38
            ...
            //gathers lightmap data from the offset in the file
        {
            ...
            if (bsp.LDR)
            {
                ...
                for (int i = 0; i < LightmapTextureLength; i++)
                {
                    ...
                    {
                        lightmapLDR[j, i] = new ColorRGBExp32(bsp.reader);
                    }
                }
            }
            if (bsp.HDR)
            {
                ...
                for (int i = 0; i < LightmapTextureLength; i++)
                {
                    for (int j = 0; j < LightmapTextureWidth; j++)
                    {
                        lightmapHDR[j, i] = new ColorRGBExp32(bsp.reader);
                    }
                }
            }
        }
        ...
        public void updateLightmap(string[] luxels, char[] mode, bool ldr)
            ...
            //mode is the rgbe settings(to edit only specific colors)
        {
            ...
            {
                if (ldr)
                    lightmapLDR[i / LightmapTextureLength, i % LightmapTextureLength].updateLuxel(luxels[i].Split(':'), mode);
                else
                    lightmapHDR[i / LightmapTextureLength, i % LightmapTextureLength].updateLuxel(luxels[i].Split(':'), mode);
            }
        }
